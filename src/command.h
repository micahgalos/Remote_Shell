...
class Command : public Base{
	...
	bool construct() {
		...
		if (comm[0] == "exit") {  exit(EXIT_SUCCESS); }   //this is if any other term comes 
		...
		// it changes your time to the one acceptable for the execvp call, 
		// if its not there you cant do th execvp call, 
		...
		if (pid > 0) {
			...
			int childStatus;
			if (waitpid(pid, &childStatus, 0) == -1) { perror("wait"); }
			if (WEXITSTATUS(childStatus) != 0) { return false; }	
		}
		...
		// test cases of child
            ...
            if(dup2(fileInput,  STDIN_FILENO) == -1) {  
				// specific documentation for input
				perror("we are using dup2");
				return false;
			}

			if(dup2(fileOutput,  STDOUT_FILENO) == -1) { 
				//this is specific documentation for output 
				perror("we are using dup2");
				return false;
			}
