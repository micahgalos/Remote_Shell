...
void rShell::show() { //starting screen of the command, the first line
    cout << "$ ";
}
...
void rShell::userSplitting() {
    ...
    string tokening;
    while(istream >> tokening) {
        listUser.push_back(tokening);
    }
}
    ...
    for (unsigned int g = 0; g < userInput.size(); ++g) {
        switch (userInput[g]) {
            case ';':  //these are cases and I was looking at for the spacing 
                if (g > 0) {
                    (userInput[g - 1] != ' ') ? userInput.insert(g, " ") : userInput;
                }
		...
		for (unsigned int g = temp; g < listUser.size(); ++g) {
			...
			if ((g + 1) < listUser.size()) {
				if (!parenthesis.empty()) { //first we find parenthesis
					if (listUser[g] == ")") {
						if (!parenthesis.empty()) { //and the stack lets us know if we done
							parenthesis.pop(); // we pop the stack once we see an ( and once we see a ) we know we are done
						}	
						...
						if (listUser[g + 1] != "(") { 
//acounting for the parenthesis in different locations
							if (listUser[g] == ";" || listUser[g] == "&&" || listUser[g] == "||" || listUser[g] 							   == "<" || listUser[g] == ">" || listUser[g] == ">>" || listUser[g] == "|" ) {
...
//saved what you pushed in the commandline so you dont have to reset it
					...
					if (listUser[g] == ";" || listUser[g] == "&&" || listUser[g] == "||" || listUser[g] == "<" || 
							listUser[g] == ">" || listUser[g] == ">>" || listUser[g] == "|") {
						connectors = new connectingStrings(listUser[g]);
						connectingList.push_back(connectors);
					}
...
/*void rShell::connectParsing(unsigned int temp, vector<connectingStrings*> & connectingList) {
	...
	for (unsigned int g = temp; g < listUser.size(); ++g) {
		...
		if ((g + 1) < listUser.size()) {
			if (!parenthesis.empty()) { 
				...
				if (listUser[g + 1] != "(") { 
					if (listUser[g] == ";") {
						connectors = new connectingStrings(";");
						connectingList.push_back(connectors); 
					}
					else if (listUser[g] == "&&") {
						connectors = new connectingStrings("&&");
	...
	//helps our exection in the main file so that we can know the process of the parser, where it clears first then gets rid of #, next checking for spacing and the split makes sure each command is one index and the connectParsing is the final step that runs final step from
	...
	bool next = true;
		for (unsigned int g = 0; g < listUser.size(); ++g) {
			...
			listUser[g] == ">" || listUser[g] == ">>" || listUser[g] == "|")){			
				...
				if ((g + 1) < listUser.size()) {
					cmd = new Command(); //we do it again for the second time
					...
					else {	//still need to create this in base file 		
						cmd->comm.push_back(listUser[g]);
        	...
        	for (unsigned int g = 0; g < connectingList.size(); ++g) { //this is the iteration through the command and it takes note of 			the connectors that it sees  
				...
				if (treeDesign.empty()) {   //tree starts out being empty b/c nothing in it
					if (connectingList[g]->templist.empty()) { //this case assumes that there is no parentheses within 						the command such as echo a; echo b  
						...
						connects = new Semi_colon(leftside, rightside); //connector has all the commands to push ba						   -ck into the tree
						...
						if (connectingList[g]->templist.empty()) {     
							...
							leftside = treeDesign.back(); //saying that there is something in the commandtree b							   ecause you didnt clear like asked in the end  
							treeDesign.pop_back();  //you have to make hand side the back if the tree isnt clea							   red to start with 
							...
							leftside = treeDesign.back(); //assuming parthesis and there is something in the co							   mmand tree     
							treeDesign.pop_back(); // this forms the command from the left hand side 
							rightside =designtree(connectingList[g]->templist); //assumes the parentheses are 							   on the right
							connects = new Semi_colon(leftside, rightside); //connector that has all the comman							   ds to push back into the tree
							treeDesign.push_back(connects); //puts the commands output into a tree vector to sh                                                        ow the answer
						}
					}
			}
